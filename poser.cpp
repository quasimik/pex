...
DEFINE_int32(keypoint_scale,            4,              "Scaling of the (x,y) coordinates of the final pose data array, i.e. the scale of the (x,y)"
...
DEFINE_string(net_resolution,           "160x-1",       "Multiples of 16. If it is increased, the accuracy potentially increases. If it is"
                                                        " decreased, the speed increases. For maximum speed-accuracy balance, it should keep the"
                                                        " closest aspect ratio possible to the images or videos to be processed. Using `-1` in"
                                                        " any of the dimensions, OP will choose the optimal aspect ratio depending on the user's"
                                                        " input value. E.g. the default `-1x368` is equivalent to `656x368` in 16:9 resolutions,"
                                                        " e.g. full HD (1980x1080) and HD (1280x720) resolutions.");
DEFINE_int32(scale_number,              2,              "Number of scales to average.");
...
DEFINE_double(render_threshold,         0.25,           "Only estimated keypoints whose score confidences are higher than this threshold will be"
...
public:
        ...
        if (datumsPtr != nullptr && !datumsPtr->empty())
        {
            ...
            auto person = 0;
                ...
                for (auto bodyPart = 0 ; bodyPart < poseKeypoints.getSize(1) ; bodyPart++)
                {
                    ...
                    const auto score = poseKeypoints[{person, bodyPart, 2}];
                    //if (score >= 0.25)
                    {
                        valueToPrint += std::to_string(bodyPart) + ":\t";
                        valueToPrint += std::to_string(x) + "\t";
                        valueToPrint += std::to_string(y) + "\t";
                        valueToPrint += std::to_string(score);
                        op::log(valueToPrint);
                    }
                }
                op::log("\n");
    ...
    void getKeypoints(const int person, const std::shared_ptr<std::vector<UserDatum>>& datumsPtr)
    {
    }
};
